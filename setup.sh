...
docker sandbox exec "$SANDBOX_NAME" bash -c "command -v corepack >/dev/null 2>&1 || sudo npm install -g corepack && sudo corepack enable && corepack prepare pnpm@${PNPM_VERSION} --activate && cd $REPO_ROOT && CI=true pnpm install --frozen-lockfile"
